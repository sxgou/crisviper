...
### Position-aware gap penalties / 位置依赖的 gap 惩罚
...
| Region / 区域 | $s(p)$ | Effective gap_open | 效果 |
|--------|--------|-------------------|------|
| Cut site / 切割位点 | 1.0 | -2.0 | 鼓励开 gap |
| Flank / 侧翼 (±3 bp) | 2.0 | -4.0 | 中等抑制 |
| Conserved / 保守区 | 2.0–6.0 | -4.0 to -12.0 | 强烈抑制（`--far-scale`） |
...
**Gap exit bonus.** 从 gap 状态切回 match 时额外施加惩罚。效果是抑制碎片化的短 match 区域，将小的 match 片段"推入"相邻 gap，合并为连续 indel。数学上，在 M 递推的 max 中，来自 Ix/Iy 的分支加上 gap_exit_bonus（负值）：
...
cutsite ±3 bp 范围外的点突变被矫正回 reference 碱基。紧邻 gap 的突变不受此规则影响——这些突变可能参与了修复过程，保留它们避免假阴性。
